...
extern "C" {
	...
	double calculate_obs_without_xi(int _C, int _N, int _M, int _J, int _P, int** _c_list, double* _knots,
		int** _fail_ind, double** _sur_interval, double** _sur_cov,
		double** _X, double* _Y, int* _nu, int* _int_eml,
		double** _lambda, double** _beta, double** _pi) {
		...
		for (int i = 0; i < _M; i++) {
			...
			log_like = log_like + max_log + log(sum_prob);
	...
	void MCMC_iteration_DIC(int* seed, int* nc, int* MCMC_iter, int* thin_iter, int* ind_swap,
		...
		double* log_like, 
		int* ind_after_burnin, int* re_iter, double* DIC, double* WAIC, double* Time) {

		auto start_alloc = chrono::system_clock::now();
		...
		auto end_alloc = chrono::system_clock::now();
		chrono::duration<double> elapsed_seconds_alloc = end_alloc - start_alloc;
		Time[0] = elapsed_seconds_alloc.count();
		...
		auto start_N = chrono::system_clock::now();
		eml = 0;
...
#pragma omp parallel for
		...
		auto end_N = chrono::system_clock::now();
		chrono::duration<double> elapsed_seconds_N = end_N - start_N;
		Time[1] = elapsed_seconds_N.count();
		...
		Time[2] = elapsed_seconds_beta.count();
		
		///////////////////////////////////
		//  3) update lambda_{lmp}  //
		// cout << "Update lambda." << endl;
		auto start_lambda = chrono::system_clock::now();
		_update_lambda(C, M, J, knots,c_list,
			fail_ind_c, sur_time, int_eml,
			mem_pair, intentions, MCMC_Rng, lambda_prior, lam_rate_prior,
			lam_c);
		auto end_lambda = chrono::system_clock::now();
		chrono::duration<double> elapsed_seconds_lambda = end_lambda - start_lambda;
		Time[3] = elapsed_seconds_lambda.count();
		...
		Time[4] = elapsed_seconds_pi.count();

		// 5.5

		auto start_swap = chrono::system_clock::now();
		if (ind_swap[0] == 1) {

			double* pair_prob = new double[C * C];
			for (int l = 0; l < C * C; l++) {
				pair_prob[l] = 1.0 / C / C;
			}
			int role_pair1 = rand_discrete(C * C, pair_prob, MCMC_Rng);
			int role_pair2 = rand_discrete(C * C, pair_prob, MCMC_Rng);

			if (role_pair1 != role_pair2) {
				int* mem_pair_temp = new int[M];

				// swapping two randomly selected role pairs
				for (int m = 0; m < M; m++) {
					if (mem_pair[m] == role_pair1) {
						mem_pair_temp[m] = role_pair2;
					}
					else if (mem_pair[m] == role_pair2) {
						mem_pair_temp[m] = role_pair1;
					}
					else {
						mem_pair_temp[m] = mem_pair[m];
					}
				}

				// update pi and xi based on the swapped role pairs

				double** pi_temp = new double* [N];
				for (int s = 0; s < N; s++) {
					pi_temp[s] = new double[C];
				}
				...
				for (int iter = 0; iter < 100; iter++) {
					_update_pi(N, C, M,
						c_list, xi_temp,
						mem_pair_temp, MCMC_Rng,
						pi_temp);

					_update_xi(N, C,
						pi_temp, MCMC_Rng, xi_prior, 10.0,
						xi_temp);
				}

				// compute the logarithm acceptance rate to determine whether swapping these two role pairs
				// prod_(i,j) p(S_ij|pi_i)p(R_ij|pi_j) 
				// prod_i p(pi_i|xi)

				double log_rat = 0.0;
				double** sum_mem_temp = new double* [N];
				double** sum_mem_iter = new double* [N];
				for (int s = 0; s < N; s++) {
					sum_mem_temp[s] = new double[C];
					sum_mem_iter[s] = new double[C];

					for (int l = 0; l < C; l++) {
						sum_mem_temp[s][l] = xi_temp[l] - 1.0;
						sum_mem_iter[s][l] = xi[l] - 1.0;
					}
				}
				...
				for (int i = 0; i < M; i++) {
					...
					ind_mem = mem_pair_temp[i];
					l = ind_mem / C;
					m = ind_mem - C * l;
					sum_mem_temp[sender][l] = sum_mem_temp[sender][l] + 1;
					sum_mem_temp[receiver][m] = sum_mem_temp[receiver][m] + 1;

					ind_mem = mem_pair[i];
					l = ind_mem / C;
					m = ind_mem - C * l;
					sum_mem_iter[sender][l] = sum_mem_iter[sender][l] + 1;
					sum_mem_iter[receiver][m] = sum_mem_iter[receiver][m] + 1;
				}

				for (int s = 0; s < N; s++) {
					for (int l = 0; l < C; l++) {
						log_rat = log_rat + sum_mem_temp[s][l] * log(pi_temp[s][l]) - sum_mem_iter[s][l] * log(pi_c[s][l]);
					}
				}

				double sum_xi_temp = 0.0;
				double sum_xi_iter = 0.0;

				for (int l = 0; l < C; l++) {
					sum_xi_temp = sum_xi_temp + xi_temp[l];
					sum_xi_iter = sum_xi_iter + xi[l];
				}
				log_rat = log_rat + N * (lgamma(sum_xi_temp) - lgamma(sum_xi_iter));

				for (int l = 0; l < C; l++) {
					log_rat = log_rat - N * lgamma(xi_temp[l]);
					log_rat = log_rat + N * lgamma(xi[l]);
				}

				// prior part of xi
				log_rat = log_rat + (xi_prior[0] - 1.0) * log(sum_xi_temp) + (xi_prior[1] - 1.0) * log(C - sum_xi_temp);
				log_rat = log_rat - (xi_prior[0] - 1.0) * log(sum_xi_iter) - (xi_prior[1] - 1.0) * log(C - sum_xi_iter);

				if (log_rat > log(MCMC_Rng.runif())) {

					// update lambda
					double lambda_temp;
					for (int j = 0; j < J; j++) {
						lambda_temp = lam_c[j][role_pair1];
						lam_c[j][role_pair1] = lam_c[j][role_pair2];
						lam_c[j][role_pair2] = lambda_temp;
					}

					// update beta
					double beta_temp;
					for (int p = 0; p < P; p++) {
						beta_temp = beta_c[p][role_pair1];
						beta_c[p][role_pair1] = beta_c[p][role_pair2];
						beta_c[p][role_pair2] = beta_temp;
					}

					// update S and R
					for (int i = 0; i < M; i++) {
						mem_pair[i] = mem_pair_temp[i];
					}

					// update pi
					for (int s = 0; s < N; s++) {
						for (int l = 0; l < C; l++) {
							pi_c[s][l] = pi_temp[s][l];
						}
					}
				...
				delete[] mem_pair_temp;

				for (int s = 0; s < N; s++) {
					delete[] pi_temp[s];
					delete[] sum_mem_temp[s];
					delete[] sum_mem_iter[s];
				}
				delete[] pi_temp;
				delete[] sum_mem_temp;
				delete[] sum_mem_iter;
		...
		auto end_swap = chrono::system_clock::now();
		chrono::duration<double> elapsed_seconds_swap = end_swap - start_swap;
		Time[5] = elapsed_seconds_swap.count();
		...
		WAIC[0] = calculate_obs_without_xi(C, N, M, J, P, c_list, knots,
			fail_ind_c, sur_interval_c, sur_cov_c,
			X, sur_time, censor_ind, int_eml,
			lam_c, beta_c, pi_c);
		...
		log_like[0] = WAIC[0] + N * lgamma(sum_xi);

		for (int l = 0; l < C; l++) {
			log_like[0] = log_like[0] - N * lgamma(xi[l]);
			for (int s = 0; s < N; s++) {
				log_like[0] = log_like[0] + (xi[l] - 1) * log(pi_c[s][l]);
			}
		}

		auto end_loglike = chrono::system_clock::now();
		chrono::duration<double> elapsed_seconds_loglike = end_loglike - start_loglike;
		Time[6] = elapsed_seconds_loglike.count();
		...
		*/
		...
		if (ind_after_burnin[0] == 1 && MCMC_iter[0] % thin_iter[0] == 0) {

			// The posterior expected value of the joint deivance
			auto start_expdev = chrono::system_clock::now();
			...
			auto end_expdev = chrono::system_clock::now();
			chrono::duration<double> elapsed_seconds_expdev = end_expdev - start_expdev;
			Time[7] = elapsed_seconds_expdev.count();

			// cout << "At " << MCMC_iter[0] << "-th resampling, the posterior expected DIC is " << DIC[0] << endl;

			// calculate the fixed point deviance
			auto start_fixdev = chrono::system_clock::now();
			...
			*/
			...
			auto end_fixdev = chrono::system_clock::now();
			chrono::duration<double> elapsed_seconds_fixdev = end_fixdev - start_fixdev;
			Time[8] = elapsed_seconds_fixdev.count();
	...
	void MCMC_iteration(int* seed, int* nc, int* MCMC_iter, int* ind_swap,
		int* num_usr, int* num_mem, int* num_com, int* num_eml, int* num_int, int* num_cov,
		double* knots, int* com_list, double* sur_time, int* censor_ind, double* cov, int* int_eml,
		double* xi, double* lambda, double* beta,
		double* lambda_prior, double* lambda_prior_rate, double* beta_prior, double* xi_prior, int* update_pi,
		...
		int* mem_pair, double* pi, int* intentions,
		double* log_like,
		int* ind_after_burnin, double* WAIC, double* Time) {
...
#pragma omp parallel for
		...
		////////////////////////////////////////
		// 5.5) global shift on S_ij and R_ij //
		auto start_swap = chrono::system_clock::now();
		if (ind_swap[0] == 1) {
		...
		// cout << "After calculating the first part, loglike = " << log_like[0] << endl;
